...
- Business Problem:
  The business problem we are trying to solve is optimizing travel agent bookings so that they can accurately gauge when  consummers will fly to destination hotspots and make a profit while doing so. To do so we will build a model that will  take historical travel data and use that to let travel agents know how far in advance they should prebook hotel rooms in popular travel locations.
...
- Business Success Criteria: 
  Success criteria for this business case would be that travel agents can use the application to streamline their booking process and also use the data provided to increase profitability.

- Ethical Framework Questions: 
  - How could your system negatively impact individuals?
    As this system is based on historical data, some years might have had outside factors that skew the data and while we tried to remove outliers there is still a chance that a booking date will be shown that is not the best possible date for a possible destination.
  
  - Who is most vulnerable and why? 
    The most vulnerable people are the travel agents who would use the application, if they try to prebook rooms expecting a lot of sales in the coming months they might not reach the expected amount of sales and will either lose money or not profit as much as possible.
  ...
  Yes, the important input factors will need to be explained
  
  - Do you need PII or can you provide group-level data? 
    There is no need for PII. 
...
- The Dataset used is obtained from: Air Canada Hackathon Data
